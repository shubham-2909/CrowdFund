...
const CreateCampaign = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dateInt = new Date(deadline).getTime()
      if (dateInt < new Date().getTime()) {
        toast.error('Please enter a valid deadline')
        return
      }
      await instance.methods
        .createCampaign(dateInt, targetInWei, title, description, image)
      ...
      >
        ...
        <div className='w-full flex justify-start items-center p-4 bg-[#8c6dfd] h-[120px] rounded-[10px]'>
          ...
          <h4 className='font-epilogue font-bold text-[25px] text-white ml-[20px]'>
            You will get 100% of the raised amount if everyone wishes.
          </h4>
        </div>
